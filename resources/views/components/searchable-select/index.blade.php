...
>
    <div
        x-data="auiSearchableSelect({
            value: @js($hasValue ? (string) $selected : null),
            property: @js($valueProperty),
            searchProperty: @js($searchModel),
            labels: @js($seedLabels),
        })"
        x-ref="wrapper"
    ...
    >
        ...
        <template x-teleport="body">
            <div
                x-ref="dropdown"
                class="aui-combobox-dropdown"
                x-show="open"
                x-cloak
                :style="{ top: top + 'px', left: left + 'px', width: width + 'px' }"
                x-on:keydown.escape.prevent="close(); $refs.trigger.focus()"
            ...
            >
                <div class="aui-combobox-search">
                    ...
                    @if ($searchModel)
                        <input
                            type="text"
                            x-ref="search"
                            class="aui-combobox-search-input"
                ...
                <div class="aui-combobox-list" x-ref="list">
                    ...
                    @if ($searchModel)
                        ...
                        @endif
                    @else
                        <p class="aui-combobox-empty" x-ref="empty" @if (filled($optionList) || $slot->isNotEmpty()) hidden @endif>{{ $emptyText }}</p>
                    @endif
                </div>
            </div>
        </template>
